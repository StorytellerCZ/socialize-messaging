...
test('clients can mark their own conversation read or leave, but cannot alter membership or presence', () => {
    let permissions;
    run('./participant-model/server/server.js', {
        Meteor: {}, User: {}, UserPresence: { onCleanup: () => {} }, console,
        ParticipantsCollection: { createIndexAsync: () => {}, allow: value => permissions = value, after: { insert: () => {}, update: () => {} } },
        ConversationsCollection: {},
    });
    const participant = { userId: 'me' };
    assert.equal(permissions.update('me', participant, ['deleted', 'read', 'updatedAt'], { $set: { deleted: true, read: true, updatedAt: new Date() } }), true);
    assert.equal(permissions.update('me', participant, ['read'], { $set: { read: true } }), true);
    assert.equal(permissions.update('me', participant, ['deleted'], { $set: { deleted: true } }), true);
    assert.equal(permissions.update('other', participant, ['read'], { $set: { read: true } }), false);
    assert.equal(permissions.update('me', participant, ['observing'], { $set: { observing: ['fake'] } }), false);
    assert.equal(permissions.update('me', { ...participant, deleted: true }, ['deleted'], { $set: { deleted: false } }), false);
});
